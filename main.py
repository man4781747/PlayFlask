...
def NextPeople(msg):
    ...
    if GameType=='TrueFalse':
        NowPeopleQuest['name'] = NowPeopleInfo['name']
    ...
    global poepleAns
    poepleAns = [0,0,0,0]
    emit('now_ans_chose', poepleAns, broadcast=True, include_self=True)
    
@socketio.on('go_ans_chose')
def go_ans_chose(msg):
    global poepleAns
    poepleAns[msg['ansChose']] += msg['count']
    emit('now_ans_chose', poepleAns, broadcast=True, include_self=True)
